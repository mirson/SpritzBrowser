...
- (id)initWithAddress:(NSString *)urlString delegate:(id<UIWebViewDelegate>)delegate {
    self = [self initWithAddress:urlString];
...
- (UIWebView*)webView {
    if(!_webView) {
        ...
        _webView.delegate = self.delegate != nil ? self.delegate : self;
...
- (void)updateToolbarItems {
    [self _updateToolbarItems:self.webView.isLoading];
}
...
- (void)_updateToolbarItems:(BOOL)isLoading
{
...
- (void)actionButtonClicked:(id)sender {
    ...
    for (id activity in [self.activityDelegate activities]) {
